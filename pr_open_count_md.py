        ...
        if (pulls_fetched == 30):
            for i in range (0, 30):
                ...
                if  time_between_pr.days<7:
                    ...
                    #repowise_pr_age_dict[repo]['MoreThan90aysold'].count += 1
                    print ("The pr= {}  is open beyond 90 days".format(response[i]['number']))
            page_no = page_no + 1
...
#        list_for_search_and_replace.append(temp_dict_md)
    ...
    if  repowise_pr_age_dict[repo]['MoreThan90daysold']['count'] != 0:
        ...
        list_for_search_and_replace.append(temp_dict)
        html_dict_1['MoreThan90DaysOldPR'] = MoreThan90Str
    final_str = str1+daysstr7+daysstr14+daysstr30+daysstr90+MoreThan90Str
